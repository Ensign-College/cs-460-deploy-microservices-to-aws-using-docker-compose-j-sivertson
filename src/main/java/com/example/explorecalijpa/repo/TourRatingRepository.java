...
public interface TourRatingRepository extends JpaRepository<TourRating, Integer> {
   ...
   * @return List of TourSummary with tour details and ratings
   */
  @Query("""
  ...
  /**
   * Find recommended tours for a specific customer (excluding tours they've already rated)
   * 
  ...
  @Query("""
   ...
   order by avg(tr.score) desc, count(tr.id) desc, tr.tour.title asc
""")
  List<TourSummary> findRecommendedForCustomer(Integer customerId, Pageable pageable);
}
